...
__global__ void coord2Morton(int P, const float3 *points, float3 minn, float3 maxx,
                             uint32_t *codes) {
...
__global__ void boxMeanDist(uint32_t P, float3 *points, uint32_t *indices, MinMax *boxes,
                            float *dists) {
...
void SimpleKNN::knn(int P, float3 *points, float *meanDists) {
    ...
    cub::DeviceReduce::Reduce(temp_storage.data().get(), temp_storage_bytes, points, result, P,
                              CustomMin(), init);
    ...
    boxMeanDist<<<num_boxes, BOX_SIZE>>>(P, points, indices_sorted.data().get(), boxes.data().get(),
                                         meanDists);
