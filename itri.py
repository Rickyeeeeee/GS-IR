...
for scene in scenes:
    ...
    images = "images_2" 
    commands = [
        f"{prefix} python train.py -m {output_path} -s {scene_path} --iterations 30000 -i {images} --data_device cpu --eval",
        f"{prefix} python baking.py -m {output_path} --checkpoint {checkpoint_30k} --bound 16.0 --occlu_res 256 --occlusion 0.4",
        f"{prefix} python train.py -m {output_path} -s {scene_path} --start_checkpoint {checkpoint_30k} --iterations 40000 -i {images} --eval --gamma --metallic --indirect",
        f"{prefix} python render.py -m {output_path} -s {scene_path} --checkpoint {checkpoint_40k} --eval --skip_train --pbr --gamma --indirect",
    ]
